...
class deloWarp extends PluginBase implements Listener {
	...
	public function onPrecess(PlayerCommandPreprocessEvent $event) {
		$message = $event->getMessage ();
		$msg = explode ( "/", $message );
		if(! isset($msg[1])) return;
		if (! isset ( $this->db [$msg [1]] )) return;
		$this->warp ( $event->getPlayer (), $this->getWarpLevel ( $msg [1] ), $this->getWarpX ( $msg [1] ), $this->getWarpY ( $msg [1] ), $this->getWarpZ ( $msg [1] ) );
		$event->getPlayer()->sendMessage(TextFormat::GREEN . "[ 서버 ] $msg[1](으)로 이동하셨습니다.");
		$event->setCancelled();
	}
		...
		if ($sender instanceof Player) {
			...
			if ($command == "워프삭제") {
				...
				if (! isset ( $args [0] )) {
					...
					unset($this->wdb[array_search($args[0], $this->wdb)]);
			...
			if($command === "워프리스트"){
				$sender->sendMessage(TextFormat::YELLOW . "워프 목록: " . $this->returnWarplists());
				return true;
			}
		} else {
		...
		if ($event->getBlock ()->getId () == Block::SIGN_POST || $event->getBlock ()->getId () == Block::WALL_SIGN) {
			...
			if ($event->getPlayer ()->isOp ()) {
				...
				if (isset ( $this->pdb [$x . "." . $y . "." . $z] )) {
					...
					$event->getPlayer ()->sendMessage ( TextFormat::GREEN . "해당 포탈을 삭제했습니다." );
	 ...
	 * 
	 */
	public function returnWarplists(){
		$lists = "";
		foreach(array_keys($this->warps) as $list){
			$lists .= $list . " ";
			return $lists;
		}
	}
	...
	public function errorMsgOp($sender) {
		if ($sender instanceof Player) {
			...
			$sender->sendMessage ( TextFormat::BLUE . "/워프 <워프포인트>" );
			$sender->sendMessage(TextFormat::BLUE . "/워프리스트");
		}
	}
